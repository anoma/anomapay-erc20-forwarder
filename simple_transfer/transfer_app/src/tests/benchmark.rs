...
/// A mint transaction generates 2 logic proofs and 1 compliance proof.
...
/// Generate a test for various concurrency.
...
async fn benchmark_mint() {
    let num: i32 = std::env::var("CONCURRENCY")
        .expect("CONCURRENCY env var must be set to an integer between 1 and infinity")
        .parse()
        .expect("CONCURRENCY env var must be set to an integer between 1 and infinity");
