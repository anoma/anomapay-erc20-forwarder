//! Deploys the ERC20 tokens the scenarios wrap: a mintable `ERC20Example` and a
//! `WETH9` funded by depositing native currency. `IERC20` reads tokens the
//! environment did not deploy itself.

use alloy::primitives::Address;
use alloy::primitives::U256;
use alloy::providers::Provider;
use alloy::sol;
use anoma_pa_testkit::environment::StateBuilder;
use anyhow::Context;

use crate::state::erc20::insert_erc20_address;

sol!(
    #[allow(missing_docs)]
    #[derive(Debug)]
    #[sol(rpc)]
    ERC20Example,
    "src/deploy/artifacts/ERC20Example.json"
);

#[inline]
pub fn erc20_example<P>(address: Address, provider: P) -> ERC20Example::ERC20ExampleInstance<P>
where
    P: Provider,
{
    ERC20Example::ERC20ExampleInstance::new(address, provider)
}

pub async fn deploy_example_erc20<P>(provider: P) -> anyhow::Result<Address>
where
    P: Provider,
{
    let deployed = ERC20Example::deploy(provider)
        .await
        .context("failed to deploy ERC20Example")?;

    Ok(*deployed.address())
}

pub async fn deploy_and_mint_example_erc20<P>(
    provider: P,
    mint_to: Address,
    amount: U256,
) -> anyhow::Result<Address>
where
    P: Provider + Clone,
{
    let deployed = ERC20Example::deploy(provider.clone())
        .await
        .context("failed to deploy ERC20Example")?;

    deployed
        .mint(mint_to, amount)
        .send()
        .await
        .context("failed to submit ERC20Example mint transaction")?
        .get_receipt()
        .await
        .context("failed to fetch ERC20Example mint receipt")?;

    Ok(*deployed.address())
}

pub async fn deploy_and_insert_example_erc20<P>(
    builder: &mut StateBuilder,
    symbol: &str,
    provider: P,
    mint_to: Address,
    amount: U256,
) -> anyhow::Result<Address>
where
    P: Provider + Clone,
{
    let address = deploy_and_mint_example_erc20(provider, mint_to, amount).await?;

    insert_erc20_address(builder, symbol, address);

    Ok(address)
}

sol!(
    #[allow(missing_docs)]
    #[derive(Debug)]
    #[sol(rpc)]
    IERC20,
    "src/deploy/artifacts/IERC20.json"
);

#[inline]
pub fn ierc20<P>(address: Address, provider: P) -> IERC20::IERC20Instance<P>
where
    P: Provider,
{
    IERC20::IERC20Instance::new(address, provider)
}

sol!(
    #[allow(missing_docs)]
    #[derive(Debug)]
    #[sol(rpc)]
    WETH9,
    "src/deploy/artifacts/WETH9.json"
);

#[inline]
pub fn weth9<P>(address: Address, provider: P) -> WETH9::WETH9Instance<P>
where
    P: Provider,
{
    WETH9::WETH9Instance::new(address, provider)
}

pub async fn deploy_weth<P>(provider: P) -> anyhow::Result<Address>
where
    P: Provider,
{
    let deployed = WETH9::deploy(provider)
        .await
        .context("failed to deploy WETH9")?;

    Ok(*deployed.address())
}

pub async fn deploy_and_mint_weth<P>(
    provider: P,
    mint_to: Address,
    amount: U256,
) -> anyhow::Result<Address>
where
    P: Provider + Clone,
{
    let deployed = WETH9::deploy(provider.clone())
        .await
        .context("failed to deploy WETH9")?;

    deployed
        .deposit()
        .value(amount)
        .send()
        .await
        .context("failed to submit WETH9 deposit transaction")?
        .get_receipt()
        .await
        .context("failed to fetch WETH9 deposit receipt")?;

    deployed
        .transfer(mint_to, amount)
        .send()
        .await
        .context("failed to submit WETH9 transfer transaction")?
        .get_receipt()
        .await
        .context("failed to fetch WETH9 transfer receipt")?;

    Ok(*deployed.address())
}

pub async fn deploy_and_insert_weth<P>(
    builder: &mut StateBuilder,
    symbol: &str,
    provider: P,
    mint_to: Address,
    amount: U256,
) -> anyhow::Result<Address>
where
    P: Provider + Clone,
{
    let address = deploy_and_mint_weth(provider, mint_to, amount).await?;

    insert_erc20_address(builder, symbol, address);

    Ok(address)
}
