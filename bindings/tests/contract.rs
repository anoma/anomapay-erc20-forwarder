...
use erc20_forwarder_bindings::addresses::{forwarder_address, forwarder_deployments_map};
...
async fn versions_of_deployed_forwarders_point_to_the_current_protocol_adapter_contract() {
    ...
    for chain in forwarder_deployments_map().keys() {
        let pa =
            protocol_adapter_address(chain.clone()).expect("Couldn't get protocol adapter address");
        ...
        let deployed_protocol_adapter = forwarder_address(chain.clone());
