...
Two scripts do the work. [`DeployERC20ForwarderMigration`](./contracts/script/migration/DeployERC20ForwarderMigration.s.sol) deploys the migration contract and makes the deployment wallet `0x61462bE56782568376f9cB069382EFa72764a407` its owner, in both environments. [`MigrateERC20ForwarderAssets`](./contracts/script/migration/MigrateERC20ForwarderAssets.s.sol) proposes the caller assignment to the committee Safe, moves the tokens as the deployment wallet, and checks the result. Both read the two forwarders from [`RecordedDeployments`](./contracts/generated/RecordedDeployments.sol) and check them against the chain: the environment's proxy owner owns the V2 forwarder, and the V2 forwarder forwards for another protocol adapter than V1. The proposal, the move and the check also check the migration contract: it holds the recorded forwarders, the deployment wallet owns it, and the owner of the v1 protocol adapter has stopped it.
...
> [!NOTE]
> Forge 1.8.3 scripts panic on forks of OP-stack chains such as Base. On those chains, append `--network ethereum` to every recipe below.
...
- [ ] Deploy the V2 forwarder of the environment and record it, as for a chain new to an environment in [`RELEASE_CHECKLIST.md`](./RELEASE_CHECKLIST.md). Its `<PROTOCOL_ADAPTER>` is the protocol adapter proxy that pa-evm records for the environment, not the v1 protocol adapter. The scripts check only that it differs from the v1 protocol adapter, so read it back:

  ```sh
  cast call <FORWARDER_V2> "getProtocolAdapter()(address)" --rpc-url <CHAIN>
  ```
...
- [ ] Simulate the deployment of the migration contract, with the deployment wallet as the sender, then deploy it. The deployment does not need the v1 stop.
  ...
  The deployment is not deterministic, so a repeated run deploys a second contract. Export the address this run reports, and use it in every step below:

  ```sh
  export MIGRATION_ADDRESS=<ADDRESS>
  ```

- [ ] Verify the migration contract on sourcify and Etherscan, so that the Safe signers can read the source of the contract they assign:
...
These steps are step 5 of the pa-evm checklist: they run after the v1 stop and before the completion run. Users cannot transact in that time. The scripts refuse the proposal before the stop, because V1 accepts the caller assignment only after it. Agree on a signing time with the committee's signers before the stop.

The pa-evm migration run sends its transactions from the deployment wallet, and so do step 6 and a kind table update in step 1. Send those only after that run has ended. The other steps can run while it copies the state.
...
2. [ ] Simulate the proposal, which simulates the Safe executing the caller assignment:

   ```sh
   export IS_PRODUCTION=<true|false>
   just contracts-simulate-migration-caller-proposal $MIGRATION_ADDRESS <PROPOSER> <CHAIN>
   ```

   Besides the checks in [How it works](#how-it-works), it requires that V1 has no emergency caller yet.

3. [ ] Propose the caller assignment to the committee Safe:

   ```sh
   just contracts-propose-migration-caller deployer $MIGRATION_ADDRESS <PROPOSER> <CHAIN>
   ```

   where `<PROPOSER>` is a Safe owner or delegate.

4. [ ] Ask the signers of `0xc703402252Ce1251aa07e0815D50060d27fdd6C4` to confirm and execute the queued transaction in the [Safe app](https://app.safe.global/home?safe=0xc703402252Ce1251aa07e0815D50060d27fdd6C4). They can read `FORWARDER_V1()`, `FORWARDER_V2()` and `owner()` on the verified migration contract. The caller assignment cannot be undone. Once executed, confirm it:

   ```sh
   cast call <FORWARDER_V1> "getEmergencyCaller()(address)" --rpc-url <CHAIN>
   ```

5. [ ] Simulate the move, with the deployment wallet as the sender:

   ```sh
   just contracts-simulate-migration-move 0x61462bE56782568376f9cB069382EFa72764a407 $MIGRATION_ADDRESS "$TOKENS" <CHAIN>
   ```
...
6. [ ] Run it:
   ...
   ```sh
   just contracts-execute-migration deployer $MIGRATION_ADDRESS "$TOKENS" <CHAIN>
   ```
...
7. [ ] Check the result, reading from the chain:

   ```sh
   just contracts-check-migration $MIGRATION_ADDRESS "$TOKENS" <CHAIN>
   ```

   It checks that the migration contract is V1's emergency caller, and that V1 holds none of the tokens. The `ERC20TokenMigrated` events record the amounts the V2 forwarder received.

8. [ ] Continue with step 6 of the pa-evm checklist. Its completion run unpauses the protocol adapter proxy, and V1 resources can then unwrap from the V2 forwarder.
